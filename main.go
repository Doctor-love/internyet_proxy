...
// Loads explicitly trusted CN from file
func loadCNAllowlist(trustedCNsFile string) (trustedCNs []string, err error) {
	if trustedCNsFile == "" {
		log.Print("WARN: No explicitly trusted CN specified")
		return trustedCNs, err
	}

	log.Print("INFO: Loading explicitly trusted CN from file: ", trustedCNsFile)
...
func includedInAllowlist(cn string) (included bool) {
	log.Print("INFO: Checking if CN is included in allowlist: ", cn)

	for _, entry := range trustedCNs {
		if cn == entry {
			log.Print("INFO: CN is included in allowlist: ", cn)
			return true
		}
	}
...
func validateCert(rawCerts [][]byte, verifiedChains [][]*x509.Certificate) (err error) {
	...
	for _, verifiedChain := range verifiedChains {
		...
		// If CN is allowlisted, we proceede with the request
		if includedInAllowlist(verifiedChain[0].Subject.CommonName) {
			return err
		}
	}

	return errors.New("WARN: Could not find a matching CN in allowlist")
}
...
func init() {
	...
	flag.StringVar(&trustedCNsFile, "cn-allowlist", "", "Path to new line separated file containg allowed CNs")
	...
	if confFromEnv == true {
		...
		serverAddress = os.Getenv("INPR_SERVER_ADDRESS")
		serverCert = os.Getenv("INPR_SERVER_CERT")
		serverKey = os.Getenv("INPR_SERVER_KEY")
		clientCAFile = os.Getenv("INPR_CLIENT_CA")
		clientCRLFile = os.Getenv("INPR_CLIENT_CRL")
		targetURL = os.Getenv("INPR_TARGET_URL")
		trustedCNsFile = os.Getenv("INPR_CN_ALLOWLIST")

		if os.Getenv("INPR_ADD_HSTS") == "true" {
			addHSTS = true
		}
	}
}
...
func main() {
	...
	trustedCNs, err = loadCNAllowlist(trustedCNsFile)
	if err != nil {
		log.Fatal("ERROR: Failed to load explicitly trusted CN: ", err)
	}
